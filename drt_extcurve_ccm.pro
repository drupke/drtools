...
;
; :Returns:
;    Array of values A(lambda)/A(V) with length equal to the number of
;    input wavelengths.
;
; :Params:
;    lambda: in, required, type=dblarr(N)
;        Wavelengths at which to compute the extinction.
;
; :Keywords:
...
;
;-
function drt_extcurve_ccm,lambda,rv=rv
  ...
  for i=0,n_elements(lambda)-1 do begin
     ...
     if (invlam[i] ge lran_ir[0] AND invlam[i] le lran_ir[1]) then begin
        ...
        print,'DRT_EXTCURVE_CCM: Out of wavelength range of CCM89.  Returning 0.'
        return,0d
     endelse
